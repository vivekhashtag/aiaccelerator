...
const C = {
  electron: "#00f5d4", // carriers / inversion layer / N-type
  gate: "#f59e0b", // gate / control
  on: "#00e676", // conducting / HIGH
  off: "#64748b", // off / depleted
  teal: "#00c9a7", // module-1 accent
  hole: "#fb7185", // holes / P-type
  ink: "#081020", // deepest fill
  panel: "#0f2138", // raised fill
  panel2: "#15294a", // elevated fill
  ...
  const row = (
    ...
    return (
      <g>
        ...
        {/* P region */}
        <rect x="48" y={top} width={depX0 - 48} height={h} rx="4" fill="rgba(251,113,133,0.12)" stroke="rgba(251,113,133,0.4)" />
        ...
        {/* N region */}
        <rect x={depX1} y={top} width={588 - depX1} height={h} rx="4" fill="rgba(0,245,212,0.10)" stroke="rgba(0,245,212,0.4)" />
  ...
  const panel = (ox: number, on: boolean) => {
    ...
    return (
      <g>
        ...
        <rect x={srcX} y={subY - 2} width="60" height="34" rx="3" fill="rgba(0,245,212,0.16)" stroke="rgba(0,245,212,0.5)" />
        <rect x={drnX} y={subY - 2} width="60" height="34" rx="3" fill="rgba(0,245,212,0.16)" stroke="rgba(0,245,212,0.5)" />
        ...
        {/* gate oxide + gate */}
        <rect x={srcX + 60} y={subY - 10} width={drnX - srcX - 60} height="8" fill="rgba(245,158,11,0.2)" stroke="rgba(245,158,11,0.4)" />
        <rect x={srcX + 60} y={subY - 34} width={drnX - srcX - 60} height="22" rx="3"
          fill={on ? "rgba(245,158,11,0.7)" : "rgba(100,116,139,0.4)"} stroke={on ? C.gate : C.off} strokeWidth="1.5" />
        ...
        {on ? (
          <g>
            ...
            <rect x={srcX + 60} y={subY - 1} width={drnX - srcX - 60} height="7" fill="rgba(0,245,212,0.25)" />
            {[0, 1, 2, 3, 4, 5].map((i) => (
          ...
          row.map((v, j) => (
            <g key={`${i}-${j}`}>
              <rect x={x0 + j * cell} y={y0 + i * cell} width={cell} height={cell}
                fill={v ? "rgba(0,201,167,0.14)" : C.ink} stroke={C.line} strokeWidth="1" />
  ...
  return (
    <DiagramFrame caption={caption} maxWidth={600}>
      <svg viewBox="0 0 600 250" width="100%" role="img" aria-label="Clock waveform with setup and hold windows">
        ...
        <rect x="250" y="110" width="100" height="40" fill="rgba(0,245,212,0.10)" stroke="rgba(0,245,212,0.4)" strokeDasharray="3 3" />
          ...
          return (
            <g key={b.v}>
              <rect x={x} y={baseY - h} width={bw} height={h} rx="4"
                fill={i === 0 ? "rgba(251,113,133,0.35)" : "rgba(0,245,212,0.30)"}
          ...
          return (
            <g key={i}>
              ...
              <rect x={x + 6} y="44" width={cw - 12} height={cw - 12} rx="6"
                fill={on ? "rgba(0,245,212,0.18)" : C.ink} stroke={on ? C.electron : C.line} strokeWidth="1.5" />
...
export function TwosComplementDiagram({ caption }: { caption?: string }) {
  const row = (y: number, label: string, bits: string, col: string) => (
    <g>
      ...
      {bits.split("").map((b, i) => (
        <g key={i}>
          <rect x={150 + i * 34} y={y - 16} width="28" height="28" rx="4" fill={b === "1" ? "rgba(0,245,212,0.16)" : C.ink} stroke={C.line} />
    ...
    <DiagramFrame caption={caption} maxWidth={600}>
      <svg viewBox="0 0 560 270" width="100%" role="img" aria-label="Bit-field layouts of FP32, FP16, BF16 and INT8">
        ...
        <g fontFamily={mono} fontSize="10">
          <rect x="96" y="14" width="12" height="12" fill="rgba(251,113,133,0.6)" /><text x="114" y="24" fill={C.muted}>sign</text>
          <rect x="170" y="14" width="12" height="12" fill="rgba(245,158,11,0.6)" /><text x="188" y="24" fill={C.muted}>exponent</text>
          <rect x="270" y="14" width="12" height="12" fill="rgba(0,245,212,0.45)" /><text x="288" y="24" fill={C.muted}>mantissa</text>
        </g>
        {formats.map((f, i) => {
          const y = 44 + i * rowH;
          const unit = fullW / 32;
          ...
          return (
            <g key={f.name}>
              ...
              <rect x={x0} y={y} width={sw} height="32" fill="rgba(251,113,133,0.55)" stroke={C.ink} />
              {f.exp > 0 && <rect x={x0 + sw} y={y} width={ew} height="32" fill="rgba(245,158,11,0.55)" stroke={C.ink} />}
              <rect x={x0 + sw + ew} y={y} width={mw} height="32" fill="rgba(0,245,212,0.40)" stroke={C.ink} />
              {/* field bit-counts */}
  ...
  return (
    <DiagramFrame caption={caption} maxWidth={600}>
      <svg viewBox="0 0 600 260" width="100%" role="img" aria-label="Ripple carry versus carry lookahead">
        ...
        <rect x="60" y="184" width="480" height="30" rx="6" fill="rgba(0,230,118,0.12)" stroke={C.on} strokeWidth="1.5" />
  ...
  return (
    <DiagramFrame caption={caption} maxWidth={560}>
      <svg viewBox="0 0 560 240" width="100%" role="img" aria-label="3-bit ripple counter from T flip-flops">
        ...
        {seq.map((s, i) => (
          <g key={s}>
            <rect x={56 + i * 56} y={172} width="46" height="30" rx="4" fill={i === 0 ? "rgba(0,245,212,0.18)" : C.ink} stroke={C.line} />
